package javaOnlineRu.L17_FutureTask;
        ...
        while (true) {
            try {
                ...
                if (!futureTask[0].isDone())
                    System.out.println("1st thread finished : " + futureTask[0].get());

                System.out.println("Waiting of 2nd thread completion");
                String txt = futureTask[1].get(200L,TimeUnit.MILLISECONDS);
                if(txt != null)
                    System.out.println("2nd thread completed : " + txt);
                System.out.println("Check of completion od 3rd thread");
                if (futureTask[2].isCancelled())
                    System.out.println("3rf thread interrupted ...");
                else if (!futureTask[2].isDone()) {
                    txt = futureTask[2].get();
                    System.out.println("3rd thread completed : "+txt);
                }
                Thread.sleep(200);
